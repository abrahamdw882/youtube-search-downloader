const proxyUrl = "https://broken-star-6439.abrahamdw882.workers.dev/?u=";
    ...
    async function fetchDownloadLinks(button, videoUrl) {
      ...
      try {
        const response = await fetch(proxyUrl + encodeURIComponent(`https://bk9.fun/download/ytmp3?url=${videoUrl}`));
        const data = await response.json();

        if (data.status && data.BK9 && data.BK9.downloadUrl) {
          data.BK9.downloadUrl.forEach(link => {
            const downloadButton = document.createElement('a');
            downloadButton.classList.add('download-button');
            downloadButton.href = link.downloadUrl;
            downloadButton.target = '_blank';
            downloadButton.innerText = `${link.quality} - ${link.format}`;
